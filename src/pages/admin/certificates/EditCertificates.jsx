...
const EditCertificate = () => {
  ...
  const existing = certificates.find((c) => (c._id || c.id)?.toString() === id);

  const [formData, setFormData] = useState({
    title: existing?.certificateTitle || existing?.title || "",

    organization: existing?.issuingOrganization || existing?.organization || "",
  ...
  useEffect(() => {
    if (existing) {
      ...
      setPreview(existing.certificateFile || existing.image || null);
    }
  }, [existing]);
